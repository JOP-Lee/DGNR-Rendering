...
from nerfstudio.utils.nerfbusters_utils import random_train_pose
import pdb
import math
...
class Pipeline(nn.Module):
    ...
    def apply_regular_loss(self, step: int, patches_density: TensorType["num_patches", "res", "res"]):
        pd = patches_density

        delta_x = pd[..., :-1, 1:] - pd[..., :-1, :-1]
        delta_y = pd[..., 1:, :-1] - pd[..., :-1, :-1]
        delta_xy = pd[..., 1:, 1:] - pd[..., :-1, :-1]
        loss = torch.mean(delta_x**2 + delta_y**2+ delta_xy**2)
        return loss
...
class VanillaPipeline(Pipeline):
    ...
    def get_train_loss_dict(self, step: int):
        ...
        if True:
            ...
            num_patches = 10
            camera_indices = torch.tensor(list(range(num_patches))).unsqueeze(-1)
            ray_bundle_patches = cameras.generate_rays(camera_indices)
            ...
            depth_patches = (
                model_outputs_patches["depth"].reshape(patch_resolution,patch_resolution, num_patches, 1)
                .permute(2, 0, 1, 3)[..., 0]
            ) 

            regular_loss = self.apply_regular_loss(step, depth_patches)
            loss_dict["regular_loss"] = loss_weight * regular_loss
