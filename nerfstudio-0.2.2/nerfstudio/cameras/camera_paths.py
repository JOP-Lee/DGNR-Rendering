...
def get_path_from_json(camera_path: Dict[str, Any]) -> Cameras:
    ...
    for camera in camera_path["frames"]:
        ...
        transform =[[0.9999755024909973,0.0049057649448513985,0.004995810799300671,2.8477561473846436],[0.0049057649448513985,0.018198668956756592,-0.9998223781585693,-1.6660473346710205],[-0.004995810799300671,0.9998223781585693,0.018174171447753906,-0.09364059567451477],[0,0,0,1]]

        transform_matrix = transform @ transform_matrix
        transform_matrix[ :3, 3] *= 0.15050221179874784

        c2w = torch.tensor(transform_matrix).view(4, 4)[:3]



        c2ws.append(c2w)

        fxs.append(camera_path["fl_x"])
        fys.append(camera_path["fl_y"])
...
def get_path_from_json2(camera_path: Dict[str, Any]) -> Cameras:
    """Takes a camera path dictionary and returns a trajectory as a Camera instance.
