...
from READ.gl.camera import Trackball
...
def get_args():
    ...
    parser.add_argument('--replay-camera', type=str, default='Data/kitti0_368/camera.xml', help='path to view_matrix to replay at given fps')
    #parser.add_argument('--replay-camera', type=str, default='', help='path to view_matrix to replay at given fps')
    parser.add_argument('--replay-fps', type=float, default=100., help='view_matrix replay fps')
...
class MyApp():
    def __init__(self, args):
        ...
        self.text_file = self.scene_data['config'].get('view_matrix')
        ...
        if self.scene_data['intrinsic_matrix'] is not None:
            ...
            new_proj_matrix = crop_proj_matrix(self.scene_data['proj_matrix'],
                                               *self.scene_data['config']['viewport_size'], *self.viewport_size)
        ...
        self.window = app.Window(width=self.viewport_size[0], height=self.viewport_size[1], visible=True,
                                 fullscreen=False)
        ...
        self.off_render = OffscreenRender(viewport_size=self.viewport_size,
        ...
        if need_neural_render:
            ...
            self.model = OGL(self.scene, self.scene_data, self.viewport_size, net_ckpt,
                             texture_ckpt, out_buffer_location=self.out_buffer_location,
                             supersampling=args.supersampling, temporal_average=args.temp_avg)
        else:
            self.model = None
        ...
        if args.replay_camera:
            ...
            self.camera_trajectory = load_camera_trajectory(self.text_file)
            # self.camera_trajectory = random_360()
    ...
    def save_screen(self, out_dir='./data/screenshots'):
        ...
        render_GT = self.text_file.replace( 'camera.xml','images_undistorted') 
         
        image_names=[]
        if 'cityscape' in render_GT:
            xmin=-22
            xmax=-16
        else:
            xmin = 0
            xmax = -4
        for item in sorted(os.listdir(render_GT),key=lambda x:int(x[xmin:xmax])):
            image_names.append(item)
        images = []

        out_dir='results/'+ self.text_file.split('/')[-2]+'/'
        os.makedirs(out_dir, exist_ok=True)
        test=True

        for i in range(0, len(self.camera_trajectory)):
            if test:
                if not (i%10==0):
                    continue
            image = self.render_frame(self.camera_trajectory[i])
            out2 = image.cpu().numpy()[..., :3][::-1, :, ::-1] * 255

            cv2.imwrite(out_dir+image_names[i],out2)
            images.append(out2)
            print(i)
    ...
    def on_draw(self, dt):
        ...
        if self.args.nearest_train:
            ni = nearest_train(self.scene_data['view_matrix'],
                               np.linalg.inv(self.scene_data['model3d_origin']) @ self.last_view_matrix)
    ...
    def on_mouse_press(self, x, y, buttons, modifiers=True):
    ...
    def on_mouse_release(self, x, y, button, modifiers=True):
        self.mouse_pressed = False
